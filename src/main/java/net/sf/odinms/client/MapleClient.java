...
public class MapleClient {
  ...
  public void doneedlog(Object ob, MapleCharacter player)
  {
    ...
    try {
      ...
      file.seek(num);
      file.writeBytes("\r\n");
      file.write(("日志信息: (id:" + player.getId() + " ,名字:" + player.getName() + " ,职业:" + player.getJob() + " ,地图:" + player.getMapId() + " 干的事: " + classname + "").getBytes());
      file.close();
 ...
 public void changeChannel(int channel) {
        ...
        try {
            String[] socket = ChannelServer.getInstance(getChannel()).getIP(channel).split(":");
            getSession().write(MaplePacketCreator.getChannelChange(InetAddress.getByName(socket[0]), Integer.parseInt(socket[1])));
        } catch (Exception e) {
