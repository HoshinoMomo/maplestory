...
public class AdminCommand {
    ...
    public static class Shutdown extends CommandExecute {
        ...
        public int execute(MapleClient c, String splitted[]) {
            ...
            if (t == null || !t.isAlive()) {
                t = new Thread(net.sf.odinms.server.ShutdownServer.getInstance());
                t.start();
            } else {
...
//            }
    ...
    public static class UnlockInv extends CommandExecute {
        ...
        public int execute(MapleClient c, String splitted[]) {
            ...
            for (java.util.Map.Entry<IItem, MapleInventoryType> eq : eqs.entrySet()) {
...
//                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.spawnNPC(npc, true));
    ...
    public static class copyInv extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            if (type == 0) {
                for (net.sf.odinms.client.inventory.IItem ii : victim.getInventory(MapleInventoryType.EQUIPPED).list()) {
                    net.sf.odinms.client.inventory.IItem n = ii.copy();
                ...
                for (int i = 0; i < equip.length; i++) {
                    if (equip[i] != 0) {
                        net.sf.odinms.client.inventory.IItem n = victim.getInventory(types).getItem((short) equip[i]).copy();
