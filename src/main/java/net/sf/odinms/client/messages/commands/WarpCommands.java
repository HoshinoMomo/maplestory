...
public class WarpCommands implements Command {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception, IllegalCommandSyntaxException {
        ...
        if (splitted[0].equals("!过来")) {
            ...
            if (victim != null) {
                ...
                try {
                    ...
                    if (loc != null) {
                        mc.dropMessage("由于GM呼唤你.你将更换频道,这可能需要几秒钟.");
        ...
        } else if (splitted[0].equals("!你过来")) {
            MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
            victim.changeMap(c.getPlayer().getMap(), c.getPlayer().getMap().findClosestSpawnpoint(c.getPlayer().getPosition()));
        } else if (splitted[0].equals("!地图")) {
            if (splitted.length == 1) {
                mc.dropMessage("格式: !地图 <地图 ID> <Portal ID>");
            } else {
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{
                    new CommandDefinition("过来", "playername", "Warps yourself to the player", 3),
                    new CommandDefinition("过来吧", "playerid", "Warps yourself to the player", 3),
                    ...
                    new CommandDefinition("地图", "mapid", "Warps you to the given mapid (use /m instead)", 5),
                    new CommandDefinition("都过来", "mapid", "Warps you and everyone in your map to the mapid specified", 5),
