...
public class CashItemFactoryA {
    ...
    private static Map<Integer, List<CashItemInfoA>> cashs = new HashMap();
    ...
    public static List<CashItemInfoA> getItems(int itemId) {
        if (cashs.containsKey(itemId)) {
            return cashs.get(itemId);
        }
        List<CashItemInfoA> Items = new ArrayList<CashItemInfoA>();
        ...
        MapleData a = dataProvider.getData("Cash.img");
            ...
            if (itemId == Integer.parseInt(b.getName())) {
                for (MapleData c : b.getChildren()) {
                    for (MapleData d : c.getChildren()) {
                        ...
                        Items.add(getItem(SN));
                    }
                }
                break;
            }
        }
        cashs.put(itemId, Items);
        return Items;
    }
